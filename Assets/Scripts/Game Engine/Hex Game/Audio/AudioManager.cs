...
namespace WeAreGladiators.Audio
{
    public class AudioManager : Singleton<AudioManager>
    {
        ...
        private AudioDataBox previousCombatTrack;
        ...
        public void PlaySound(AudioProfileType type, AudioSet set)
        {
            ...
            if (apd == null)
            {
                Debug.LogWarning("PlaySound() could not find an AudioProfileData that matches the type '" + type + "'.");
                return;
            }
            ...
            if (set == AudioSet.Die)
            {
                soundPlayed = apd.dieSounds[RandomGenerator.NumberBetween(0, apd.dieSounds.Length - 1)];
            }
            else if (set == AudioSet.Hurt)
            {
                soundPlayed = apd.hurtSounds[RandomGenerator.NumberBetween(0, apd.hurtSounds.Length - 1)];
            }
            else if (set == AudioSet.Attack)
            {
                soundPlayed = apd.meleeAttackSounds[RandomGenerator.NumberBetween(0, apd.meleeAttackSounds.Length - 1)];
            }
            else if (set == AudioSet.TurnStart)
            {
                soundPlayed = apd.turnStartSounds[RandomGenerator.NumberBetween(0, apd.turnStartSounds.Length - 1)];
            }

            if (soundPlayed == null)
            {
                Debug.LogWarning("PlaySound() did not find any valid sounds within the set '" + set + "', returning...");
                return;
            }
        ...
        private void BuildAudioPlayerFromAudioModelData(AudioModel data, AudioPlayer player)
        {
            ...
            // Randomize clip
            if (data.randomizeClip)
            {
                player.Source.clip = data.audioClips[RandomGenerator.NumberBetween(0, data.audioClips.Length - 1)];
            }
            else
            {
        ...
        public void FadeOutAllAmbience(float fadeDuration)
        {
            ...
            if (IsSoundPlaying(Sound.Ambience_Town_1))
            {
                FadeOutSound(Sound.Ambience_Town_1, fadeDuration);
            }
            ...
            if (IsSoundPlaying(Sound.Ambience_Crowd_1))
            {
                FadeOutSound(Sound.Ambience_Crowd_1, fadeDuration);
            }
        }
        ...
        public void StopMainMenuMusic(float fadeSpeed = 1f)
        {
            ...
            if (IsSoundPlaying(Sound.Music_Main_Menu_Theme_Unlooped_1))
            {
                FadeOutSound(Sound.Music_Main_Menu_Theme_Unlooped_1, fadeSpeed);
            }
            if (IsSoundPlaying(Sound.Music_Main_Menu_Theme_Looped_1))
            {
                FadeOutSound(Sound.Music_Main_Menu_Theme_Looped_1, fadeSpeed);
            }
        }
    ...
    public enum Sound
    {
        ...
        Ability_Jump_Start = 158,
        Ability_Jump_Land = 159,
