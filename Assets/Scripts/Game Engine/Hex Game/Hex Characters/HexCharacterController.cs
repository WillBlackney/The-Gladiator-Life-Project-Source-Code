...
namespace WeAreGladiators.Characters
{
    public class HexCharacterController : Singleton<HexCharacterController>
    {
        ...
        private string DetermineWeaponAttackAnimationString(HexCharacterModel character, ItemData weaponUsed)
        {
                     ...
                     weaponUsed.handRequirement == HandRequirement.TwoHanded)
            {
                ...
                if (weaponUsed.weaponAttackAnimationType == WeaponAttackAnimationType.Overhead)
                {
                    ...
                    if (random == 0)
                    {
                        ret = AnimationEventController.TWO_HAND_MELEE_ATTACK_OVERHEAD_2;
                    }
                    else
                    {
                        ret = AnimationEventController.TWO_HAND_MELEE_ATTACK_OVERHEAD_1;
                    }
                }
        ...
        [field: Header("Active Characters")]
        public List<HexCharacterModel> AllCharacters { get; } = new List<HexCharacterModel>();
        public List<HexCharacterModel> AllPlayerCharacters { get; } = new List<HexCharacterModel>();
        public List<HexCharacterModel> AllSummonedPlayerCharacters { get; } = new List<HexCharacterModel>();
        public List<HexCharacterModel> AllEnemies { get; } = new List<HexCharacterModel>();
        public List<HexCharacterModel> Graveyard { get; } = new List<HexCharacterModel>();
        ...
        public void ModifyHealth(HexCharacterModel character, int healthGainedOrLost, bool relayToCharacterData = true)
        {
            ...
            Debug.Log(character.myName + " health value = " + character.currentHealth);
            VisualEventManager.CreateVisualEvent(() => UpdateHealthGUIElements(character, finalHealthValue, StatCalculator.GetTotalMaxHealth(character)), character.GetLastStackEventParent());
        ...
        public void ModifyMaxHealth(HexCharacterModel character, int maxHealthGainedOrLost)
        {
            ...
            VisualEventManager.CreateVisualEvent(() => UpdateHealthGUIElements
        ...
        {
            Debug.Log("CharacterEntityController.UpdateHealthGUIElements() called, health = " + health + ", maxHealth = " + maxHealth);
        ...
        public void ModifyArmour(HexCharacterModel character, int armourGainedOrLost)
        {
            ...
            if (armourGainedOrLost < 0)
            {
                character.armourLostThisCombat += armourGainedOrLost;
            }

            // Set health after calculation
            character.currentArmour = finalBlockValue;

            Debug.Log(character.myName + " armour value = " + character.currentArmour);

            VisualEventManager.CreateVisualEvent(() => UpdateArmourGUIElements(character, finalBlockValue, maxArmour), character.GetLastStackEventParent());
        }
        private void UpdateArmourGUIElements(HexCharacterModel character, int armour, int maxArmour)
        {
            Debug.Log("CharacterEntityController.UpdateArmourGUIElements() called, armour = " + armour + ", maxArmour = " + maxArmour);
        ...
        public void ModifyMoraleState(HexCharacterModel character, int moraleGainedOrLost, bool relayToCharacterData = true, bool showVFX = false, bool allowRecursiveChecks = true, bool allowShatteredRally = false)
        {
            ...
            if (character.characterData.ignoreStress || PerkController.Instance.DoesCharacterHavePerk(character.pManager, Perk.Fearless))
            {
                if (view != null)
                {
                    view.moraleContentVisualParent.SetActive(false);
                }
                return;
            }
            ...
            {
                view.moraleContentVisualParent.SetActive(true);
            }
            ...
            if (character.currentMoraleState == MoraleState.Confident && moraleGainedOrLost > 0)
            {
                return;
            }
            ...
            {       
                ...
                {
                    VisualEventManager.CreateVisualEvent(() =>
                        VisualEffectManager.Instance.CreateStatusEffect(view.WorldPosition, ((MoraleState)finalMoraleState).ToString(), SpriteLibrary.Instance.GetMoraleStateSprite((MoraleState)finalMoraleState), StatusFrameType.ImageWithoutFrame), character.GetLastStackEventParent()).SetStartDelay(0.25f);
                }
                else
                {
                    VisualEventManager.CreateVisualEvent(() =>
                        VisualEffectManager.Instance.CreateStatusEffect(view.WorldPosition, ((MoraleState)finalMoraleState).ToString(), SpriteLibrary.Instance.GetMoraleStateSprite((MoraleState)finalMoraleState), StatusFrameType.ImageWithoutFrame));
                }
            }
            ...
            {
                Sprite shatteredSprite = SpriteLibrary.Instance.GetMoraleStateSprite(MoraleState.Shattered);
        ...
        private void UpdateStressGUIElements(HexCharacterModel character, int moraleState)
        {
            ...
            character.hexCharacterView.moraleIcon.sprite = stressSprite;
        ...
        public void TriggerStompAnimation(HexCharacterView view, TaskTracker cData)
        {
        ...
        public IEnumerator TriggerAoeMeleeAttackAnimationCoroutine(HexCharacterView view, ItemData weaponUsed, TaskTracker cData)
        {
            ...
            Vector2 startPos = view.WorldPosition;
            float xMod = 0.75f;
            if (view.character.currentFacing == Facing.Left)
            {
                xMod = -xMod;
            }
        ...
        public void PlayChargeEndAnimation(HexCharacterView view)
        {
            ...
            if (view.CurrentAnimation != AnimationEventController.CHARGE)
            {
                return;
            }
        ...
        public void PlayDeathAnimation(HexCharacterView view)
        {
            ...
            deathAnim = deathAnim + RandomGenerator.NumberBetween(1, view.model.TotalDeathAnims);
        ...
        public void PlayDecapitateAnimation(HexCharacterView view)
        {
            ...
            decapitateAnim = decapitateAnim + RandomGenerator.NumberBetween(1, view.model.TotalDecapitationAnims);
        ...
        public async void CharacterOnTurnStart(HexCharacterModel character)
        {
            ...
            {
                character.hasMadeDelayedTurn = true;
            }
            ...
            if (!character.hasRequestedTurnDelay)
            {
                ...
                {
                    // Notification event
                    VisualEventManager.CreateVisualEvent(() => VisualEffectManager.Instance.CreateStatusEffect(view.WorldPosition, "Poisoned!", PerkController.Instance.GetPerkIconDataByTag(Perk.Poisoned).passiveSprite, StatusFrameType.CircularBrown)).SetEndDelay(0.5f);
                ...
                if (PerkController.Instance.DoesCharacterHavePerk(character.pManager, Perk.Burning) && character.currentHealth > 0)
                {
                    ...
                    if (PerkController.Instance.DoesCharacterHavePerk(character.pManager, Perk.FireVulnerability))
                    {
                        fireDamage = (int) (fireDamage * 1.5f);
                    }
                ...
                {
                    // Notification event
                    VisualEventManager.CreateVisualEvent(() => VisualEffectManager.Instance.CreateStatusEffect(view.WorldPosition, "Cut Artery!", PerkController.Instance.GetPerkIconDataByTag(Perk.CutArtery).passiveSprite, StatusFrameType.CircularRed)).SetEndDelay(0.5f);
                ...
                {
                    // Notification event
                    VisualEventManager.CreateVisualEvent(() => VisualEffectManager.Instance.CreateStatusEffect(view.WorldPosition, "Cut Neck Vein!", PerkController.Instance.GetPerkIconDataByTag(Perk.CutNeckVein).passiveSprite, StatusFrameType.CircularRed)).SetEndDelay(0.5f);
        ...
        public void CharacterOnTurnEnd(HexCharacterModel character, bool firstTimeDelay = false)
        {
            ...
            if (firstTimeDelay)
            {
                CombatLogController.Instance.CreateCharacterTurnDelayEntry(character);
            }
            else
            {
                CombatLogController.Instance.CreateCharacterTurnEndEntry(character);
            }
            ...
            if (!firstTimeDelay)
            {
                ...
                if (PerkController.Instance.DoesCharacterHavePerk(character.pManager, Perk.Abusive) && character.currentHealth > 0)
                {
                    ...
                    if (allies.Count > 0)
                    {
                        ...
                        {
                            CombatController.Instance.CreateMoraleCheck(ally, new StressEventData(-1, -1, 75), true, false);
                        }
                ...
                if (PerkController.Instance.DoesCharacterHavePerk(character.pManager, Perk.DragonAspect) && character.currentHealth > 0)
                {
                    ...
                    if (enemies.Count > 0)
                    {
                        // Status Notif 
                        VisualEventManager.CreateVisualEvent(() =>
                            VisualEffectManager.Instance.CreateStatusEffect(view.WorldPosition, "Dragon Aspect!", PerkController.Instance.GetPerkIconDataByTag(Perk.DragonAspect).passiveSprite, StatusFrameType.CircularBrown)).SetEndDelay(0.5f);
                ...
                if (PerkController.Instance.DoesCharacterHavePerk(character.pManager, Perk.FieryPresence) && character.currentHealth > 0)
                {
                    ...
                    if (enemies.Count > 0)
                    {
                        // Status Notif 
                        VisualEventManager.CreateVisualEvent(() =>
                            VisualEffectManager.Instance.CreateStatusEffect(view.WorldPosition, "Fiery Presence!", PerkController.Instance.GetPerkIconDataByTag(Perk.FieryPresence).passiveSprite, StatusFrameType.CircularBrown)).SetEndDelay(0.5f);
                ...
                {
                    VisualEventManager.CreateVisualEvent(() =>
                        VisualEffectManager.Instance.CreateStatusEffect(view.WorldPosition, "Regeneration!", PerkController.Instance.GetPerkIconDataByTag(Perk.Regeneration).passiveSprite, StatusFrameType.CircularBrown)).SetEndDelay(0.5f);
                ...
                // Rapid Cloaking
                if (PerkController.Instance.DoesCharacterHavePerk(character.pManager, Perk.Elusive) &&
                    !PerkController.Instance.DoesCharacterHavePerk(character.pManager, Perk.Stealth) &&
                    character.currentHealth > 0)
                {
                    // Text Notif
                    VisualEventManager.CreateVisualEvent(() =>
                        VisualEffectManager.Instance.CreateStatusEffect(view.WorldPosition, "Rapid Cloaking", PerkController.Instance.GetPerkIconDataByTag(Perk.Elusive).passiveSprite, StatusFrameType.CircularBrown)).SetEndDelay(0.5f);
            ...
            if (firstTimeDelay)
            {
                character.hasRequestedTurnDelay = true;
            }
        ...
        {
            Debug.Log("CharacterEntityController.SetCharacterActivationPhase() called for " + character.myName + ", new phase = " + newPhase);
        ...
        public void ModifyActionPoints(HexCharacterModel character, int energyGainedOrLost, bool showVFX = false, bool updateEnergyGuiInstantly = true)
        {
            ...
            if (showVFX && view != null)
            {
                ...
                if (energyGainedOrLost > 0)
                {
                    // Status notification
                    VisualEventManager.CreateVisualEvent(() =>
                        VisualEffectManager.Instance.CreateStatusEffect(view.WorldPosition, "Action Points +" + energyGainedOrLost));
        ...
        public List<LevelNode> GetCharacterZoneOfControl(HexCharacterModel character)
        {
            ...
            Debug.Log("HexCharacterController.GetCharacterZoneOfControl() found " + zone.Count + " in " + character.myName + " zone of control");
            return zone;
        }
        ...
        public LevelNode GetCharacterBackTile(HexCharacterModel character)
        {
            ...
            foreach (LevelNode h in mRangeTiles)
            {
                ...
                if (character.currentFacing == Facing.Right &&
                    h.GridPosition.x < character.currentTile.GridPosition.x &&
                    h.GridPosition.y == character.currentTile.GridPosition.y)
                {
                    ret = h;
        ...
        {
            List<LevelNode> auraTiles = GetCharacterAura(character);
        ...
        public bool IsCharacterEngagedOrEngagingInMelee(HexCharacterModel character, int engagingEnemies = 1)
        {
            ...
            if (engagedEnemies >= engagingEnemies)
            {
                bRet = true;
            }
            else
        ...
        public int CalculateFlankingHitChanceModifier(HexCharacterModel attacker, HexCharacterModel target)
        {
            ...
            if (IsCharacterFlanked(target))
            {
                ...
                // Additonal +10 Accuracy for characters with Opportunist perk
        ...
        public void HandleTearDownCombatScene()
        {
            ...
            DestroyOnSceneChange[] destroyables = FindObjectsOfType<DestroyOnSceneChange>();
