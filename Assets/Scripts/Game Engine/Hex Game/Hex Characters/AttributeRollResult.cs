...
namespace WeAreGladiators.Characters
{
    ...
    public class PerkTreeData
    {
        ...
        {
            if (PerkController.Instance == null || hasGeneratedTree)
            {
                return;
            }
            hasGeneratedTree = true;
            ...
            for (int i = 0; i < prospects.Count; i++)
            {
                    ...
                    perk.perkTreeTier == 5 && t5.Count < 3)
                {
                    ...
                    if (perk.perkTreeTier == 1)
                    {
                        t1.Add(newPerk);
                    }
                    else if (perk.perkTreeTier == 2)
                    {
                        t2.Add(newPerk);
                    }
                    else if (perk.perkTreeTier == 3)
                    {
                        t3.Add(newPerk);
                    }
                    else if (perk.perkTreeTier == 4)
                    {
                        t4.Add(newPerk);
                    }
                    else if (perk.perkTreeTier == 5)
                    {
                        t5.Add(newPerk);
                    }
                }
        ...
        public void HandleAdjustTreeIfStartingPerkChoiceAlreadyMade(HexCharacterData myCharacter)
        {
            ...
            {
                if (pt.perk.Data.perkTreeTier == 1)
                {
                    tierOnePerks.Add(pt.perk.perkTag);
                }
            }
