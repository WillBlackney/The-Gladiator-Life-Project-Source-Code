        ...
        private void TriggerKingsChoiceEffect(KingChoiceData data)
        {
            ...
            else if (data.effect == KingChoiceEffectType.ModifyHealth)
            {
                ...
                if (data.healthGainedOrLost < 0)
                {
                    ...
                    VisualEventManager.Instance.CreateVisualEvent(() =>
                    VisualEffectManager.Instance.CreateOldDamageEffect(playerModel.transform.position, Mathf.Abs(data.healthGainedOrLost)));
                }
                else if (data.healthGainedOrLost > 0)
                {
                    ...
                    // Damage Text Effect VFX
                    VisualEventManager.Instance.CreateVisualEvent(() =>
                    VisualEffectManager.Instance.CreateOldDamageEffect(playerModel.transform.position, Mathf.Abs(data.healthGainedOrLost), true, false));
