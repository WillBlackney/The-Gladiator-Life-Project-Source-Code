...
namespace CardGameEngine
{
    public class VisualEventManager : Singleton<VisualEventManager>
    {
        ...
        private void PlayNextEventFromQueue()
        {
            ...
            /*  
            if (EventQueue.Count > 0 &&
                ((EventQueue[0].eventType == VisualEventType.Single && EventQueue[0].isPlaying == false ) || (EventQueue[0].isPlaying && EventQueue[0].eventType == VisualEventType.StackParent)) &&
                paused == false &&
                currentEventPlaying == false)
            {
                if(EventQueue[0].eventType == VisualEventType.StackParent)
                {
                    StartEventStackPlaying(EventQueue[0]);
                }
                else
                    PlayEventFromQueue(EventQueue[0]);
            }
        ...
        private IEnumerator PlayEventFromStackEvent(VisualEvent childEvent, VisualEvent parentEvent)
        {
            ...
            else if (parentEvent.EventStack.Count == 0)
            {
                ...
                if (parentEvent.EventStack.Count == 0 && EventQueue.Contains(parentEvent))
                {
                    //parentEvent.myCharacter.myCurrentEventStack = null;
                    RemoveEventFromQueue(parentEvent);
                }
        ...
        // Create Events
        #region
        public VisualEvent CreateVisualEvent(Action eventFunction, CoroutineData cData, QueuePosition position = QueuePosition.Back, float startDelay = 0f, float endDelay = 0f, EventDetail eventDetail = EventDetail.None, VisualEvent parentEvent = null)
        {
            ...
            VisualEvent vEvent = new VisualEvent(eventFunction, cData, startDelay, endDelay, eventDetail, VisualEventType.Single);

            if (parentEvent != null)
                parentEvent.AddEventToStack(vEvent);

            else if (position == QueuePosition.Back)
            {
        ...
        public VisualEvent CreateVisualEvent(Action eventFunction, QueuePosition position = QueuePosition.Back, float startDelay = 0f, float endDelay = 0f, EventDetail eventDetail = EventDetail.None, VisualEvent parentEvent = null)
        {
            VisualEvent vEvent = new VisualEvent(eventFunction, null, startDelay, endDelay, eventDetail, VisualEventType.Single);
        ...
        {
            VisualEvent vEvent = new VisualEvent(null, null, 0, delayDuration, EventDetail.None, VisualEventType.Single);
