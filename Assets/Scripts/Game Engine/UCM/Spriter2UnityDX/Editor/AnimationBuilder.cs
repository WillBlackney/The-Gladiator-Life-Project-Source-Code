...
namespace Spriter2UnityDX.Animations
{
    ...
    public class AnimationBuilder : Object
    {
        ...
        private readonly string AnimationsPath;

        private readonly IDictionary<Transform, string> ChildPaths = new Dictionary<Transform, string>();
        private readonly AnimatorController Controller;
        private readonly IDictionary<string, SpatialInfo> DefaultBones;
        private readonly IDictionary<string, SpriteInfo> DefaultSprites;
        private readonly IDictionary<int, IDictionary<int, Sprite>> Folders;
        private bool ModdedController;
        private readonly IDictionary<string, AnimationClip> OriginalClips = new Dictionary<string, AnimationClip>();
        ...
        public AnimationBuilder(
            ScmlProcessingInfo info, IDictionary<int, IDictionary<int, Sprite>> folders,
        ...
        )
        {
            ...
            foreach (Object item in GetOrigClips())
            {
        ...
        public void Build(Animation animation, IDictionary<int, TimeLine> timeLines)
        {
            ...
            Dictionary<string, Transform> pendingTransforms = new Dictionary<string, Transform>(Transforms); //This Dictionary will shrink in size for every transform
            foreach (MainLineKey key in animation.mainlineKeys)
            {
                //that is considered "used"
                Dictionary<int, List<TimeLineKey>> parentTimelines = new Dictionary<int, List<TimeLineKey>>();
                Queue<Ref> brefs = new Queue<Ref>(key.boneRefs ?? new Ref [0]);
                while (brefs.Count > 0)
                {
                    Ref bref = brefs.Dequeue();
                    if (bref.parent < 0 || parentTimelines.ContainsKey(bref.parent))
                    {
                        TimeLine timeLine = timeLines[bref.timeline];
                ...
                foreach (Ref sref in key.objectRefs)
                {
                    TimeLine timeLine = timeLines[sref.timeline];
                    Transform sprite;
                    if (pendingTransforms.TryGetValue(timeLine.name, out sprite))
                    {
                        float defaultZ = sref.z_index;
                        List<TimeLineKey> parentTimeline;
                ...
                foreach (KeyValuePair<string, Transform> kvPair in pendingTransforms)
                {
                    ...
                    {
                        AnimationCurve curve = new AnimationCurve(new Keyframe(0f, 0f, inf, inf));
            ...
            if (OriginalClips.ContainsKey(animation.name))
            {
                ...
                AnimationClip oldClip = OriginalClips[animation.name];
                AnimationEvent[] cachedEvents = oldClip.events;
            ...
            else
            {
                switch (S2USettings.ImportStyle)
                {
                    ...
                    case AnimationImportOption.SeparateFolder:
                        ...
                        {
                            int splitIndex = AnimationsPath.LastIndexOf('/');
                            string path = AnimationsPath.Substring(0, splitIndex);
                            string newFolder = AnimationsPath.Substring(splitIndex + 1);
            ...
            if (!ArrayUtility.Contains(Controller.animationClips, clip))
            {
                ...
                AnimatorState state = GetStateFromController(clip.name); //Find a state of the same name
                if (state != null)
                {
        ...
        {
            string childPath = GetPathToChild(child);
            foreach (KeyValuePair<ChangedValues, AnimationCurve> kvPair in GetCurves(timeLine, defaultInfo, parentTimeline))
            {
                ...
                switch (kvPair.Key)
                {
                    ...
                    case ChangedValues.Sprite:
                        TextureController swapper = child.GetComponent<TextureController>();
                        if (swapper == null)
                        {
                            //Add a Texture Controller if one doesn't already exist
                            swapper = child.gameObject.AddComponent<TextureController>();
                            SpriteInfo info = (SpriteInfo) defaultInfo;
                            swapper.Sprites = new[]
                            {
                                Folders[info.folder][info.file]
                            };
                        }
        ...
        private void SetAdditionalCurves(Transform child, MainLineKey[] keys, TimeLine timeLine, AnimationClip clip, float defaultZ)
        {
            ...
            List<Keyframe> kfsZ = new List<Keyframe>();
            bool changedZ = false;
            bool active = child.gameObject.activeSelf; //If the sprite or bone isn't present in the mainline,
            List<Keyframe> kfsActive = new List<Keyframe>(); //Disable the GameObject if it isn't already disabled
            string childPath = GetPathToChild(child);
            foreach (MainLineKey key in keys)
            {
                //If it is present, enable the GameObject if it isn't already enabled
                Ref mref = ArrayUtility.Find(key.objectRefs, x => x.timeline == timeLine.id);
                if (mref != null)
                {
                    if (defaultZ == inf)
                    {
            ...
            if (kfsZ.Count > 0)
            {
                ...
                if (!positionChanged)
                {
                    SpatialInfo info = timeLine.keys[0].info; //If these curves don't actually exist, add some empty ones
        ...
        {
            foreach (TimeLineKey key in timeLine.keys)
            {
                //Create a keyframe for every key on its personal TimeLine
                curve.AddKey(key.time, infoValue(key.info));
            }
            int lastIndex = animation.looping ? 0 : timeLine.keys.Length - 1; //Depending on the loop type, duplicate the first or last frame
        ...
        private void SetKeys(AnimationCurve curve, TimeLine timeLine, ref Sprite[] sprites, Animation animation)
        {
            foreach (TimeLineKey key in timeLine.keys)
            {
                ...
                SpriteInfo info = (SpriteInfo) key.info;
            ...
            int lastIndex = animation.looping ? 0 : timeLine.keys.Length - 1;
            SpriteInfo lastInfo = (SpriteInfo) timeLine.keys[lastIndex].info;
        ...
        private AnimatorState GetStateFromController(string clipName)
        {
            ...
            {
                AnimatorState state = GetStateFromMachine(layer.stateMachine, clipName);
                if (state != null)
                {
                    return state;
                }
        ...
        private AnimatorState GetStateFromMachine(AnimatorStateMachine machine, string clipName)
        {
            ...
            foreach (ChildAnimatorStateMachine cmachine in machine.stateMachines)
            {
                AnimatorState state = GetStateFromMachine(cmachine.stateMachine, clipName);
                if (state != null)
                {
                    return state;
                }
        ...
        {
            Dictionary<ChangedValues, AnimationCurve> rv = new Dictionary<ChangedValues, AnimationCurve>(); //This method checks every animatable property for changes
            foreach (TimeLineKey key in timeLine.keys)
            {
                //And creates a curve for that property if changes are detected
                SpatialInfo info = key.info;
                if (!info.processed)
                {
                    SpatialInfo parentInfo = null;
                    if (parentTimeline != null)
                    {
                        TimeLineKey pKey = parentTimeline.Find(x => x.time == key.time);
                        if (pKey == null)
                        {
                            List<TimeLineKey> pKeys = parentTimeline.FindAll(x => x.time < key.time);
                ...
                SpriteInfo scontrol = defaultInfo as SpriteInfo;
                if (scontrol != null && !rv.ContainsKey(ChangedValues.Sprite))
                {
                    SpriteInfo sinfo = (SpriteInfo) info;
